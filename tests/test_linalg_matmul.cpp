...
TEST("linalg/matmul_shapes_and_backward") {
    ...
    std::vector<float> seed(C.value().size(), 1.0f);
    C.backward(seed);
...
TEST("linalg/matmul_numeric_grad_single_entry") {
    ...
    Variable A = tensor({0.5f,-0.3f,0.1f, 0.2f,-0.7f,1.1f}, {2,3}, /*requires_grad=*/true);
    Variable B = tensor({0.9f, -0.4f, 0.3f, 1.2f,
                         -0.1f, 0.5f, -0.6f, 0.8f,
                         0.2f, 0.3f, -0.7f, -0.9f}, {3,4}, /*requires_grad=*/true);
    auto C = matmul(A,B);
    // loss L = sum(C^2) -> dL/dC = 2*C
    std::vector<float> seed(C.value().size());
    for (size_t i=0;i<seed.size();++i) seed[i] = 2.0f * C.value()[i];
    C.backward(seed);
    ...
    float Lpos = 0.0f; for (float v : Cpos.value()) Lpos += v*v;

    Avals = A.value();
    Avals[0] -= eps;
    auto Aneg = tensor(Avals, {2,3}, false);
    auto Cneg = matmul(Aneg, B);
    float Lneg = 0.0f; for (float v : Cneg.value()) Lneg += v*v;

    float fd = (Lpos - Lneg) / (2*eps);
    float an = A.grad()[0];
    ASSERT_NEAR(an, fd, 1e-3f);
}
...
TEST("linalg/matmul_associativity_numeric") {
    ...
    Variable B = tensor({ 1.1f, -0.4f,
                          0.3f,  0.7f}, {2,2}, /*requires_grad=*/false);
    Variable C = tensor({ 0.9f,  0.0f,
                          0.2f,  1.3f}, {2,2}, /*requires_grad=*/false);
...
TEST("linalg/matmul_grad_wrt_B_numeric") {
    ...
    Variable A = tensor({ 0.2f, -0.1f, 0.3f,
                         -0.5f,  0.7f, 0.8f}, {2,3}, /*requires_grad=*/true);
    Variable B = tensor({-0.6f, 0.9f,
                          0.4f, 0.1f,
                          0.0f, 0.5f}, {3,2}, /*requires_grad=*/true);
...
TEST("linalg/matmul_with_identity_keeps_matrix") {
    ...
    for (size_t i=0;i<A.value().size();++i) {
        ASSERT_NEAR(R1.value()[i], A.value()[i], 1e-6f);
    }
