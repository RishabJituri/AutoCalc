...
TEST("ops/stats/reduce_max_keepdims_false_and_backward_ties") {
    ...
    ASSERT_NEAR(Y.value()[1], 2.0, 1e-12);

    Y.backward({1.0,1.0});
    ASSERT_NEAR(X.grad()[1], 0.5, 1e-12);
    ASSERT_NEAR(X.grad()[2], 0.5, 1e-12);
    ASSERT_NEAR(X.grad()[5], 1.0, 1e-12);
}

TEST("ops/stats/logsumexp_matches_reference") {
    std::vector<double> xv = { -5.0, 0.0, 5.0, 10.0 };
    Variable X(xv, {2,2}, /*requires_grad=*/false);
    auto LSE = logsumexp(X, /*axes=*/{1}, /*keepdims=*/false);
    for (int r=0;r<2;++r) {
        double a = X.value()[r*2+0], b = X.value()[r*2+1];
        double m = std::max(a,b);
        double exp_sum = std::exp(a-m) + std::exp(b-m);
        double ref = m + std::log(exp_sum);
        ASSERT_NEAR(LSE.value()[r], ref, 1e-12);
    }
}

TEST("ops/stats/softmax_rows_sum_to_one") {
    std::vector<double> xv = { 0.1, -0.2, 0.3,
                               2.0,  0.0, -1.0 };
    Variable X(xv, {2,3}, false);
    auto Y = softmax(X, /*axis=*/1);
    for (int r=0;r<2;++r) {
        double s=0.0; for (int c=0;c<3;++c) s += Y.value()[r*3 + c];
        ASSERT_NEAR(s, 1.0, 1e-12);
    }
}

TEST("ops/stats/argmax_lastdim_returns_indices") {
    std::vector<double> xv = { 0.2, 0.5, 0.3,
                               1.0, -2.0, 3.0 };
    Variable X(xv, {2,3}, false);
