...
BatchNorm2d::BatchNorm2d(std::size_t C, double eps, double momentum)
  : C(C), eps(eps), momentum(momentum),
    gamma(std::vector<double>(C, 1.0), {C}, /*requires_grad=*/true),
    beta (std::vector<double>(C, 0.0), {C}, /*requires_grad=*/true),
    running_mean(std::vector<double>(C, 0.0), {C}, /*requires_grad=*/false),
    running_var (std::vector<double>(C, 1.0), {C}, /*requires_grad=*/false)
{
...
Variable BatchNorm2d::forward(const Variable& X) {
  ...
  if (training()) {
    for (std::size_t c=0; c<C; ++c) {
      ...
      running_mean.n->value[c] = (1.0 - momentum) * running_mean.n->value[c] + momentum * mean[c];
      running_var .n->value[c] = (1.0 - momentum) * running_var .n->value[c] + momentum * var[c];
    }
  } else {
  ...
  // forward normalize
  std::vector<double> inv_std(C, 0.0);
  for (std::size_t c=0; c<C; ++c) inv_std[c] = 1.0 / std::sqrt(var[c] + eps);
  ...
  for (std::size_t w=0; w<W; ++w) {
    std::size_t i = b*xstr[0] + c*xstr[1] + h*xstr[2] + w*xstr[3];
    double xhat = (X.n->value[i] - mean[c]) * inv_std[c];
...
out->backward = [this, Xn = X.n, Gn = gamma.n, Bn = beta.n, xs, mean, inv_std, NHW,
  ...
  if (Gn && Gn->requires_grad) {
    std::vector<double> dgamma(this->C, 0.0);
    ...
    for (std::size_t w=0; w<xs[3]; ++w) {
      ...
      double xhat = (Xn->value[i] - mean[c]) * inv_std[c];
  ...
  if (Bn && Bn->requires_grad) {
    std::vector<double> dbeta(this->C, 0.0);
    for (std::size_t b=0; b<xs[0]; ++b)
    for (std::size_t c=0; c<this->C; ++c)
    for (std::size_t h=0; h<xs[2]; ++h)
    for (std::size_t w=0; w<xs[3]; ++w) {
  ...
  if (Xn && Xn->requires_grad) {
    std::vector<double> sum_dy(this->C, 0.0), sum_dy_xhat(this->C, 0.0);
    ...
    for (std::size_t w=0; w<xs[3]; ++w) {
      std::size_t i = b*xstr_local[0] + c*xstr_local[1] + h*xstr_local[2] + w*xstr_local[3];
      double xhat = (Xn->value[i] - mean[c]) * inv_std[c];
      double ggamma = (Gn ? Gn->value[c] : 1.0);
      double term = double(NHW) * o->grad[i] - sum_dy[c] - xhat * sum_dy_xhat[c];
      Xn->grad[i] += (ggamma * inv_std[c] / double(NHW)) * term;
    }
  }
};
