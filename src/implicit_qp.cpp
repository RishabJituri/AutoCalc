...
Variable QPSolveEq(const Variable& H, const Variable& q,
  ...
  auto Hval = H.n->value; // keep original H (before ridge); backward will add eps again
  auto Aval = A.n->value;
  auto qval = q.n->value;
  auto bval = b.n->value;
  std::weak_ptr<Node> ow = out, Hw = H.n, qw = q.n, Aw = A.n, bw = b.n;
  out->backward = [ow, Hw, qw, Aw, bw, Hval, Aval, qval, bval, N, M, eps_pd]() {
    ...
    // Recompute forward solve (cheap, small dims assumed)
    std::vector<double> y_star, lambda_star, H_L;
    // Add ridge for PD
    std::vector<double> Hpd = Hval;
    for (std::size_t i=0;i<N;++i) Hpd[i*N + i] += std::max(1e-12, eps_pd);
    qp_solve_eq_forward(Hpd, N, qval, Aval, M, bval, y_star, lambda_star, H_L);

    // Solve adjoint (KKT)^T * [u_y; u_lambda] = [o->grad; 0]
    // Since KKT is symmetric (for equality-only), it's the same as forward:
    // H u_y + A^T u_lambda = bar_y;  A u_y = 0
    ...
    if (Hn && Hn->requires_grad) {
      // dL/dH = sym(u_y * y_star^T)
      for (std::size_t i=0;i<N;++i) {
        for (std::size_t j=0;j<N;++j) {
          double g = 0.5 * (u_y[i]*y_star[j] + u_y[j]*y_star[i]);
          Hn->grad[i*N + j] += g;
        }
      }
    }
    ...
    if (An && An->requires_grad) {
      // dL/dA = u_lambda * y_star^T + lambda_star * u_y^T
      for (std::size_t i=0;i<M;++i) {
        for (std::size_t j=0;j<N;++j) {
